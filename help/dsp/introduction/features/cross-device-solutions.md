...
Advertising Cloud DSP整合 [!DNL LiveRamp] 和 [!DNL Adobe Device Co-op] 可讓您將對象延伸至人員的所有已知裝置，而不只是您的品牌已追蹤的裝置。 整合也提供跨所有裝置的頻率限定和歸因測量。
...
如果您想要啟用跨裝置歸因測量，請連絡您的 [!DNL Adobe] 客戶經理。 針對 [!DNL Adobe Device Co-op] 帳戶，您需要提供您的簽名 [!DNL Adobe Device Co-op] 合約與Experience Cloud [!DNL Organization ID] (先前稱為 [!DNL IMS org ID])。

若要查看廣告商帳戶是否已設定為使用裝置圖表進行歸因測量：

1. 在主功能表中，按一下 **[!UICONTROL Settings]>[!UICONTROL Advertiser]**.
1. 將游標停留在廣告商行上，然後按一下 **[!UICONTROL Edit]**.
1. 在 [!UICONTROL Integrations] 區段，請參閱 [!UICONTROL Cross-Device Attribution] 設定為作用中。
