...
\* *[!UICONTROL Account]* 只有在貴組織已設定為 [跨帳戶報告](report-about.md#cross-account-reporting):  [!UICONTROL Custom], [!UICONTROL Site], [!UICONTROL Segment], [!UICONTROL Geo], [!UICONTROL Device], [!UICONTROL Frequency (by Impression)]，和 [!UICONTROL Conversion]. 請連絡您的 [!DNL Adobe] 客戶經理，以取得跨帳戶報告的詳細資訊。
...
**[!UICONTROL Cross Device Level]:**  (全部 [!UICONTROL Custom], [!UICONTROL Conversion], [!UICONTROL Device], [!UICONTROL Geo], [!UICONTROL Segment]，和 [!UICONTROL Site] 報表 [!UICONTROL Conversion Metrics] 或 [!UICONTROL Custom Goals] 欄；僅適用於具有跨裝置歸因的廣告商)追蹤轉換的層級： *[!UICONTROL People]* 或 *[!UICONTROL Household]*.

深入了解 [跨裝置解決方案](/help/dsp/introduction/features/cross-device-solutions.md).

**[!UICONTROL Cross-Device Breakout]:** (全部 [!UICONTROL Custom], [!UICONTROL Conversion], [!UICONTROL Device], [!UICONTROL Geo], [!UICONTROL Segment]，和 [!UICONTROL Site] 報表 [!UICONTROL Conversion Metrics] 或 [!UICONTROL Custom Goals] 欄；僅適用於具有跨裝置歸因的廣告商)要納入報表的跨裝置轉換詳細程度。 如果您想要詳細的劃分，可以選擇最多三個級別，每個級別都將包含在單獨的列中。

* *[!UICONTROL Total People (TP)]:* 包含總轉換，包括相同裝置轉換和跨裝置轉換（如果適用）。 在報告中，「[!UICONTROL (tp)]「 」會附加至轉換量度名稱和規則類型。

* *[!UICONTROL Same Device (SD)]:* 僅包含轉換路徑中僅追蹤單一裝置的轉換。 在報告中，「[!UICONTROL (sd)]「 」會附加至轉換量度名稱和規則類型。

* *[!UICONTROL Cross Device (XD)]:* 僅包括在轉換路徑中追蹤多個裝置的轉換。 在報告中，「[!UICONTROL (xd)]「 」會附加至轉換量度名稱和規則類型。
