...
**[!UICONTROL Placement Cap]:** （選用）不重複裝置或人員的次數(取決於指定 [!UICONTROL Cross Device Level])將從版位中獲得廣告。 選項包括 *[!UICONTROL Unlimited]* 或每天、每週或每月的特定數量。
...
**[!UICONTROL Topic Targeting]:** (可選；可用於配置有 [!DNL Comscore] 和 [!DNL Grapeshot] 區段)特定區段名稱或ID [!DNL Comscore] 和 [!DNL Grapeshot] 包含作為目標。 此功能可能需額外付費。 若要啟用此功能並設定主題區段，請連絡您的 [!DNL Adobe] 客戶經理。
