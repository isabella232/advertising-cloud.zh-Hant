...
| [!UICONTROL Always Max Bid & Highest Viewability Rate (Moat – GroupM)] | 透過套件層級最佳化，預算分配會以最高可檢視率來排列版位優先順序。<br><br>如果支出目標達到，拍賣評估將優先考慮可見度。提交的出價一律為設定的「最高出價」，但如果刊登版位花費不錯，預計的可見度臨界值會更嚴格。 | 促銷活動類型：品牌推廣<br><br>基準：最高可檢視率<br><br>廣告類型：僅互動式前段<br><br><b>注意：</b>此目標將一律使用版位層級的「最高競價」。<br><br>當來自協力廠商的資料通知演算法時，此 [!DNL Moat] 設定的運作效果最佳。只有在已為促銷活動啟用[!DNL Moat]整合時，才使用此目標。 |
| [!UICONTROL Always Max Bid & Highest Viewability Rate ([!DNL Moat] – MRC)] | 透過套件層級最佳化，預算分配會以最高可檢視率來排列版位優先順序。<br><br>如果支出目標達到，拍賣評估將優先考慮可見度。提交的出價一律為設定的「最高出價」，但如果刊登版位花費不錯，預計的可見度臨界值會更嚴格。 | 促銷活動類型：品牌推廣<br><br>基準：最高可檢視率<br><br>廣告類型：僅互動式前段<br><br><b>注意：</b>此目標將一律使用版位層級的「最高競價」。<br><br>當來自協力廠商的資料通知演算法時，此 [!DNL Moat] 設定的運作效果最佳。只有在已為促銷活動啟用[!DNL Moat]整合時，才使用此目標。 |
